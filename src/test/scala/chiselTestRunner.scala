...
private class ChiselTestRunner(instructions: List[Int], settings: List[TestSetting], c: Tile, d: PeekPokeTester[Tile], terminalAddress: Addr, maxSteps: Int) {

    /** Currently unused as it is quite nontrivial to figure out whether read data is actually used. Still, with the necessary scaffolding these can be correlated with reg writes to figure out if they
      * are valid or not.
      */
...
object ChiselTestRunner {
    ...
    def apply(binary: List[Int], settings: List[TestSetting], terminalAddress: Addr, maxSteps: Int, testName: String): Either[String, (Option[String], List[CircuitTrace])] = {
